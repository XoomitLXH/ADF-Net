...
class Lap_Pyramid_Conv(nn.Module):
    def __init__(self, num_high=1, device=0, color_high_freq=True):
    ...
    def pyramid_decom_color(self, img):
        img = img.to(self.device)
        ...
        if self.color_high_freq:
            ...
            for _ in range(self.num_high):
                ...
                if up.shape[2] != current.shape[2] or up.shape[3] != current.shape[3]:
                    up = nn.functional.interpolate(up, size=(current.shape[2], current.shape[3]))
                diff = current - up
                color_high_freq.append(diff)
                current = down
            
                        
            return color_high_freq[0] if color_high_freq else current
        else:
                            
            return self.pyramid_decom(img)[0]
